...
//        $counts = $books->count();
        $counts = User::find($userid)->booking->count();
        $booking_sum = User::find($userid)->booking->sum('total_price');
...
//            't_rows','b_date','i_amount','payments','p_date','confirm_after','userid','pays','b_id'));
        return view('user_account', compact('user','books' ,'countries',
                        'recents','blogs','populars','counts','booking_sum'));
    }
...
//        $b_id = $request->input('b_id');
            $bookAll = DB::table('bookings')->where('booking_code', $booking_code)->first();

//            $bookAll = DB::table('bookings')->where('id','=', $b_id)->first();
//        dd($bookAll);
                $bookDate = $bookAll->booking_date;
                $datePlan = $bookAll->date_plan;
                ...
                $bstatus= $bookAll->status;
		
                $b_id = $bookAll->id;

//        $pays = Booking::find($booking_code);
        $pays = DB::table('payments')->where('booking_code', $booking_code)->get();
//        $pays = Booking::find($b_id);
//        dd($pays);
//            foreach ($pays->payments as $pay) {
            foreach ($pays as $pay) {
                $paymentss[] = $pay->payment;
                $p_dates[] = $pay->payment_date;
//                    ->format('d-M-Y');
            }

        return response()->json([$paymentss, $p_dates, $bookDate, $confirm_after, $i_amount,$t_row,$ins_amount,$bstatus,$datePlan]);
    }
